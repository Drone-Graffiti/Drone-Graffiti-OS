...
void NavEKF2_core::ResetPosition(void)
{
    ...
    if (PV_AidingMode != AID_ABSOLUTE) {
        ...
        } else if (imuSampleTime_ms - extNavLastReadTime_ms < 250) {
            // use the range beacon data as a second preference
            stateStruct.position.x = extNavDataDelayed.pos.x;
            stateStruct.position.y = extNavDataDelayed.pos.y;
            // set the variances from the beacon alignment filter
            P[6][6] = sq(frontend->_xPosNoise);
            P[7][7] = sq(frontend->_yPosNoise);
            posTimeout = false;
            lastPosPassTime_ms = imuSampleTime_ms;
    ...
    if (fuseVelData || fusePosData || fuseHgtData) {
        ...
        if (PV_AidingMode == AID_NONE) {
            ...
            if (gpsPosAccuracy > 0.0f) {
                ...
                R_OBS[3] = sq(constrain_float(frontend->_xPosNoise, 0.01f, 10.0f));
                R_OBS[4] = sq(constrain_float(frontend->_yPosNoise, 0.01f, 10.0f));
            } else {
...
void NavEKF2_core::selectHeightForFusion()
{
    ...
    if (extNavDataToFuse && (activeHgtSource == HGT_SOURCE_EV)) {
        ...
        posDownObsNoise = sq(constrain_float(frontend->_zPosNoise, 0.01f, 10.0f));
