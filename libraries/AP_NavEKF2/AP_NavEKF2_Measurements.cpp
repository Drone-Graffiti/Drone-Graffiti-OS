...
void NavEKF2_core::writeExtNavData(const Vector3f &sensOffset, const Vector3f &pos, const Quaternion &quat, float posErr, float angErr, uint32_t timeStamp_ms, uint32_t resetTime_ms)
{
    ...
    //timeStamp_ms -= frontend->_extNavDelay_ms + localFilterTimeStep_ms/2;
    // Prevent time delay exceeding age of oldest IMU data in the buffer
    //timeStamp_ms = MAX(timeStamp_ms,imuDataDelayed.time_ms);
    //extNavDataNew.time_ms = timeStamp_ms;

    extNavDataNew.time_ms = imuSampleTime_ms - frontend->_extNavDelay_ms;

    extNavLastReadTime_ms = imuSampleTime_ms;
...
void NavEKF2_core::writeVisionSpeed(const Vector3f &vel, uint32_t timeStamp_ms)
{
    ...
    // Correct for the average intersampling delay due to the filter updaterate
    //timeStamp_ms -= localFilterTimeStep_ms/2 + frontend->_extNavDelay_ms;
    // Prevent time delay exceeding age of oldest IMU data in the buffer
    //timeStamp_ms = MAX(timeStamp_ms,imuDataDelayed.time_ms);
    //visionSpeedNew.time_ms = timeStamp_ms;

    visionSpeedNew.time_ms = imuSampleTime_ms - frontend->_extNavDelay_ms;
