...
void GCS_MAVLINK::handle_vision_speed_estimate(mavlink_message_t *msg)
{
    ...
    log_vision_velocity_estimate_data(m.usec, m.x, m.y, m.z);
}
...
void GCS_MAVLINK::log_vision_velocity_estimate_data(const uint64_t usec,
                                                    const float vx,
                                                    const float vy,
                                                    const float vz)
{
    DataFlash_Class::instance()->Log_Write("VISV", "TimeUS,RemTimeUS,VX,VY,VZ",
                                           "ssmmm", "FF000", "QQfff",
                                           (uint64_t)AP_HAL::micros64(),
                                           (uint64_t)usec,
                                           (double)vx * 100,
...
void GCS_MAVLINK::handle_common_message(mavlink_message_t *msg)
{
    ...
    case MAVLINK_MSG_ID_DATA64:
        ...
        if (msg != nullptr) {
            mavlink_data64_t packet_data64;
            mavlink_msg_data64_decode(msg, &packet_data64);    
            if (packet_data64.type == 66) {
                //result = MAV_RESULT_ACCEPTED;
                int32_t raw_data[12];
                for (int i=0; i<12; i++) {
                    raw_data[i] = (packet_data64.data[4 * i] << 24) | 
                                (packet_data64.data[4 * i + 1] << 16) | 
                                (packet_data64.data[4 * i + 2] << 8) | 
                                packet_data64.data[4 * i + 3];
                }

                DataFlash_Class::instance()->Log_Write("GRAF", "TimeUS,P1,P2,P3,P4,TL,TR,ALT,VX,VY,R1,R2,R3",
                                           "smmmmmmmmmmmm", "F000000000000", "Qffffffffffff",
                                           (uint64_t)AP_HAL::micros64(),
                                           (double)(raw_data[0]) / 1000.0,
                                           (double)(raw_data[1]) / 1000.0,
                                           (double)(raw_data[2]) / 1000.0,
                                           (double)(raw_data[3]) / 1000.0,
                                           (double)(raw_data[4]) / 1000.0,
                                           (double)(raw_data[5]) / 1000.0,
                                           (double)(raw_data[6]) / 1000.0,
                                           (double)(raw_data[7]) / 1000.0,
                                           (double)(raw_data[8]) / 1000.0,
                                           (double)(raw_data[9]) / 1000.0,
                                           (double)(raw_data[10]) / 1000.0,
                                           (double)(raw_data[11]) / 1000.0);
            }
        }
        //mavlink_msg_command_ack_send_buf(msg, chan, MAVLINK_MSG_ID_DATA64, result);
        break;
    }
